...
# Args
# -------------------------
parser = argparse.ArgumentParser("LSTM SOC training (with auto by-file validation split)")
parser.add_argument('--data-dir', type=str, required=True, help='Training folder (CSV files)')
parser.add_argument('--outdir', type=str, default='soc_model_lstm', help='Where to save checkpoints/plots')
...
parser.add_argument('--layer', type=int, default=2, help='Number of LSTM layers')
parser.add_argument('--dropout', type=float, default=0.1, help='Dropout rate')
parser.add_argument('--robust-scaler', action='store_true', help='Use RobustScaler (default StandardScaler)')
...
def train_model(train_list, val_list):
    Scaler = RobustScaler if args.robust_scaler else StandardScaler
    scaler = Scaler()
    ...
    print(f"[model] LSTM with hidden={args.hidden}, layers={args.layer}, dropout={args.dropout}")
    print(f"[model] Input dim={in_dim}, Total params={sum(p.numel() for p in model.parameters()):,}")

    for e in range(args.epochs):
        model.train()
        train_losses = []
        for d in train_list:
            xt = torch.from_numpy(d['X']).float().unsqueeze(0).to(device)  # (1, seq_len, features)
            yt = torch.from_numpy(d['y']).float().unsqueeze(0).to(device)  # (1, seq_len)
            pred = model(xt)
